...
imds = imageDatastore(dataFolderTrain, ...
...
for i = 1:numel(idx)
    subplot(4,2,i)
    img=readimage(imdsTrain,idx(i));
    img=lib.downsample(img,down_level);
    img=uint8(imresize(img,down_level,'nearest'));
    imshow(img)
...
batchSize = 10;
[pairImage1,pairImage2,pairLabel] = getTwinBatch(imdsTrain,batchSize,'down_level',down_level);
...
while iteration < numIterations && ~monitor.Stop
    ...
    [X1,X2,pairLabels] = getTwinBatch(imdsValidation,miniBatchSize,'down_level',down_level);
...
save('net_on_brodatz_by16.mat','net','fcParams')

%% Test accuracy over test pairs.
accuracy = zeros(1,4);
accuracyBatchSize = 300;

for i = 1:4
    % Extract mini-batch of image pairs and pair labels
    [X1,X2,pairLabelsAcc] = getTwinBatch(imds,accuracyBatchSize,'down_level',down_level);
...
%% Load the generalization test data and labels from folder names.

dataFolderGen=fullfile("img_data/brodatz/patches/generalize");
imdsGen = imageDatastore(dataFolderGen, ...
    IncludeSubfolders=true, ...
    LabelSource="foldernames");
...
numClasses = numel(unique(imdsGen.Labels));

%% Test accuracy over generalization test pairs.
accuracy = zeros(1,4);
accuracyBatchSize = 300;

for i = 1:4
    % Extract mini-batch of image pairs and pair labels
    [X1,X2,pairLabelsAcc] = getTwinBatch(imdsGen,accuracyBatchSize,'down_level',down_level);
...
[XTest1,XTest2,pairLabelsTest] = getTwinBatch(imdsGen,testBatchSize,'down_level',down_level);
