...
experiment.run.displayLevelStart(SessionSettings);

for iTrial = 1:nTrials    
    response = experiment.run.runTrial(SessionSettings, iTrial);    
    responseVector(iTrial) = response;
end

pCorrect=mean(SessionSettings.diffpair == responseVector)*100;

experiment.run.saveCurrentLevel(SessionSettings, responseVector, currentLevel);
