...
if(nargin < 4)
    exp_settings = experiment.grouping.run.loadCurrentSession(subject_name, exp_type);
else
    exp_settings = experiment.grouping.run.loadCurrentSession(subject_name, exp_type, condition, sessionNumber, levelNumber);
end
...
experiment.grouping.run.runLevel(SessionSettings);