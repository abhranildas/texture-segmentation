...
usecolor = 1; % 1 = color, 0 = grayscale
%
...
nbin = 256/bw1;
b1 = zeros(nbin+1,1); % histogram edges
for i = 1:nbin+1
  b1(i) = (i-1)*bw1;
end
b1(nbin+1) = 256;
nbin = 256/bw2;
b2 = zeros(nbin+1,1); % histogram edges
for i = 1:nbin+1
  b2(i) = (i-1)*bw2;
end
b2(nbin+1) = 256;
nbin = 256/bw3;
b3 = zeros(nbin+1,1); % histogram edges
for i = 1:nbin+1
  b3(i) = (i-1)*bw3;
end
b3(nbin+1) = 256;
...
for k = 1:nimgk
  ...
  imk = fil_dsmp(imk,ppd,pd,w,lev,filter,3);    % filter and downsample
  for j = k+1:nimgj
    num = num2str(j-1+mnk);
    name = append('FC',num,'.png');
    % name = append('B',num,'.gif');
    ...
    imj = fil_dsmp(imj,ppd,pd,w,lev,filter,3);    % filter and downsample    
%
% run trials
    i = 1;
    while i <= ntrl
...
% compute responses
      ...
      rhd(k,j,i) = Rh(ptch1,ptch2,b1,b2,b3,3,abr);
...
% compute responses
      ...
      rhs(k,j,i) = Rh(ptch1,ptch2,b1,b2,b3,3,abr);
...
elseif train == 2 % train on all responses and save bound
  ...
  results_e =  classify_normals(vres,vred,...
    'input_type','samp'); % 3 edge histogram   
  % results_pe =  classify_normals([vrps,vres],[vrpd,vred],...
  %   'input_type','samp'); % 4 power and edge
  % results_he =  classify_normals([vrhs,vres],[vrhd,vred],...
  %   'input_type','samp'); % 5 histogram and edge
  results_ph =  classify_normals([vrps,vrhs],[vrpd,vrhd],...
    'input_type','samp'); % 6 power and histogram
  results =  classify_normals([vrps,vrhs,vres],[vrpd,vrhd,vred],...
    'input_type','samp'); % 7 all
