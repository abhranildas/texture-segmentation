...
% 1 = linear RGB, 2 = RBG (e.g., Fabric textures),
% 3 = linear grayscale (e.g., Brodatz textures)
itype = 3;  
...
% normalizationb
ntype = 3;   % normalization type (0 = none, 3 = mean)
...
% color and edge historams
load("cdfs-9-10-12_6.mat"); % natural image cdfs
...
binflag = 2; % 0 = linear, 1 = srgb, 2 = abr 
...
% edge gradient histogram bins
eflag = 1;   % 1 = marginals, 2 = joint
thresh = 100;  % gradient threshold
nbgm = 16;    % number of gradient-magnitude bins
nbgo = 16;    % number of gradient-orientation bins
stpth = floor(360/nbgo);
bgo = (-180:stpth:180);
bgm = mk_bins(eg,Ng,nbgm); % use cumulative histogram of gradients

% Retinal resolution and location
lev = 8;                % level of downsampling (1,2,4,8,or 16)
...
% compute responses
      ...
      red(k,j,i) = Re(gptch1,gptch2,psz,thresh,bgo,bgm,eflag);      
...
% compute responses
      ...
      res(k,j,i) = Re(gptch1,gptch2,psz,thresh,bgo,bgm,eflag);
